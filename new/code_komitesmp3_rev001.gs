 ...
 * Revisi 1.01 / backend rev001
 * Google Apps Script + Google Spreadsheet
 * Tahun Pelajaran 2026/2027
 * Nominal Komite: Rp50.000 / bulan / siswa
 *
 * Fokus rev001:
 * - Cache data agar daftar siswa/rekap lebih cepat.
 * - Paket bootstrap sekali ambil setelah login.
 * - Rekap Bulan / Semester 1 / Semester 2 / Tahun Ajaran.
 * - ID transaksi dari perangkat untuk retry yang idempotent.
 * - Refresh paksa cache tanpa mengubah struktur Spreadsheet.
 */
...
function setupAplikasi() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    classRows.push(['K07' + letter, 'VII ' + letter, 'Isi Nama Wali VII ' + letter, 'wali7' + letter.toLowerCase(), 'AKTIF']);
  }
...
function buildStaffBootstrapForSession_(session, payload) {
  ...
  const kelas = session.role === 'WALI' ? clean_(session.kelas) : clean_(payload.kelas);
...
function listStudents_(payload) {
  ...
  if (kelas) result = result.filter(s => clean_(s.kelas) === clean_(kelas));
...
function buildStudentList_(session, rawStudents, paidSets, periods) {
  ...
  if (session.role === 'WALI') students = students.filter(s => clean_(s.KELAS) === clean_(session.kelas));
...
function aggregateSummary_(scope, kelas, rawStudents, paidSets, includeList, amount) {
  const students = rawStudents.filter(s => !kelas || clean_(s.KELAS) === clean_(kelas));
...
function classes_() {
  ...
  if (cached) return cached;
  const result = rowsAsObjects_(getSheet_(APP.SHEETS.KELAS))
...
function readAndNormalizeStudents_() {
  ...
  if (changedId || changedStatus) SpreadsheetApp.flush();
