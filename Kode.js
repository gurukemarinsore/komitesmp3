 ...
 * Revisi 1.13 / backend rev005
 * Google Apps Script + Google Spreadsheet
 * Tahun Pelajaran 2026/2027
 * Nominal Komite: Rp50.000 / bulan / siswa
 *
 * Fokus rev005 / Versi 1.13:
 * - Login Siswa memakai NISN + PASSWORD_SISWA universal per kelas.
 * - Migrasi schema PENGGUNA menyisipkan PASSWORD_SISWA setelah PASSWORD tanpa merusak data lama.
 * - Bootstrap staf tetap mengirim status dan riwayat pembayaran ringkas untuk UI local-first.
 * - Pembayaran multi-bulan tetap disinkronkan batch dan disimpan per bulan.
 * - ID transaksi tetap internal untuk idempotensi, log, cetak, dan pembatalan.
 */
...
const APP = Object.freeze({
  ...
  HEADERS: {
    ...
    PENGGUNA: ['ID_USER', 'USERNAME', 'PASSWORD', 'PASSWORD_SISWA', 'NAMA', 'ROLE', 'KELAS', 'STATUS'],
...
function setupAplikasi() {
  ...
  const pengguna = ensurePenggunaSheet_(ss);
  ...
  pengguna.getRange('B:D').setNumberFormat('@');
  ...
  setWidths_(pengguna, [90, 150, 140, 150, 230, 120, 110, 95]);
  ...
  ss.toast('Setup selesai. Kolom PASSWORD_SISWA sudah disiapkan. Isi password universal pada setiap kelas.', 'Komite SMPN 3 Jayapura', 10);
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'addPaymentsBatch': data = addPaymentsBatch_(payload); break;
...
function loginStudent_(payload) {
  ...
  const password = clean_(payload.password);
  if (!nisn || !password) throw new Error('NISN dan kata sandi wajib diisi.');

  const students = activeStudents_();
  const student = students.find(r => clean_(r.NISN) === nisn);
  if (!student) throw new Error('NISN atau kata sandi salah.');

  const kelas = normalizeClassName_(student.KELAS);
  const penggunaSheet = getSheet_(APP.SHEETS.PENGGUNA);
  const header = penggunaSheet.getRange(1, 1, 1, Math.max(1, penggunaSheet.getLastColumn())).getValues()[0].map(clean_);
  if (!header.includes('PASSWORD_SISWA')) {
    throw new Error('Password siswa belum disiapkan. Jalankan setupAplikasi() pada backend.');
  }

  const users = rowsAsObjects_(penggunaSheet);
  const classAccount = users.find(r =>
    clean_(r.STATUS).toUpperCase() === 'AKTIF' &&
    normalizeClassName_(r.KELAS) === kelas &&
    clean_(r.PASSWORD_SISWA)
  );
  if (!classAccount) throw new Error('Kata sandi siswa untuk ' + kelas + ' belum diatur.');
  if (clean_(classAccount.PASSWORD_SISWA) !== password) throw new Error('NISN atau kata sandi salah.');
  ...
  logActivity_(session.nisn, 'LOGIN_SISWA', '', session.idSiswa);
  return {
...
function buildStaffBootstrapForSession_(session, payload) {
  ...
  const paymentSnapshot = bootstrapPaymentSnapshot_(periods, session);
  const paidSets = paymentSnapshot.paidSets;
...
function addPaymentsBatch_(payload) {
  const session = requireSession_(payload.token, ['BENDAHARA']);
  const items = Array.isArray(payload.items) ? payload.items : [];
  if (!items.length) throw new Error('Tidak ada pembayaran yang akan disimpan.');
  if (items.length > 36) throw new Error('Terlalu banyak pembayaran dalam satu proses. Silakan simpan dalam beberapa tahap.');

  const periods = periods_();
  const periodKeys = new Set(periods.map(p => periodKey_(p.bulan, p.tahun)));
  const students = activeStudents_();
  const studentMap = {};
  students.forEach(s => studentMap[clean_(s.ID_SISWA)] = s);
  const amount = Number(settings_().NOMINAL_KOMITE || 50000);
  ...
  try {
    ...
    for (let r = 1; r < values.length; r++) {
      const trx = idx.ID_TRANSAKSI >= 0 ? clean_(values[r][idx.ID_TRANSAKSI]) : '';
      if (trx) existingByTrx[trx] = values[r];
      if (clean_(values[r][idx.STATUS]).toUpperCase() === 'LUNAS') {
        const key = clean_(values[r][idx.ID_SISWA]) + '|' + periodKey_(clean_(values[r][idx.BULAN]).toUpperCase(), Number(values[r][idx.TAHUN]));
        existingPaid[key] = values[r];
      }
    }
    ...
    items.forEach(item => {
      const rawClientId = clean_(item && item.clientTransactionId);
      let clientId = rawClientId;
      try {
        const idSiswa = clean_(item && item.idSiswa);
        const bulan = clean_(item && item.bulan).toUpperCase();
        const tahun = Number(item && item.tahun);
        const requestedTrx = normalizeClientTransactionId_(rawClientId);
        clientId = requestedTrx || rawClientId;
        const selectedDate = normalizePaymentDate_(item && item.tanggalPembayaran);
        if (!idSiswa || !bulan || !tahun) throw new Error('Siswa dan bulan pembayaran wajib dipilih.');
        if (!periodKeys.has(periodKey_(bulan, tahun))) throw new Error('Periode pembayaran tidak valid.');

        const student = studentMap[idSiswa];
        if (!student) throw new Error('Data siswa tidak ditemukan atau tidak aktif.');

        if (requestedTrx && existingByTrx[requestedTrx]) {
          const row = existingByTrx[requestedTrx];
          const status = clean_(row[idx.STATUS]).toUpperCase();
          if (status === 'BATAL') throw new Error('Transaksi ini pernah diterima tetapi sudah dibatalkan.');
          results.push({ clientId: requestedTrx, ok: true, transaction: paymentRowToResponse_(row, idx, true) });
          return;
        }

        const paidKey = idSiswa + '|' + periodKey_(bulan, tahun);
        if (existingPaid[paidKey]) {
          results.push({ clientId: requestedTrx || rawClientId, ok: true, transaction: paymentRowToResponse_(existingPaid[paidKey], idx, true) });
          return;
        }
        ...
        newRows.push(row);
        existingByTrx[trx] = row;
        existingPaid[paidKey] = row;
        logRows.push([new Date(), session.username, 'BAYAR_' + bulan + '_' + tahun, trx, idSiswa]);
        results.push({ clientId: requestedTrx || rawClientId, ok: true, transaction: paymentRowToResponse_(row, idx, false) });
      } catch (err) {
        results.push({ clientId: clientId, ok: false, message: errorMessage_(err) });
      }
    });

    if (newRows.length) {
      paymentSheet.getRange(paymentSheet.getLastRow() + 1, 1, newRows.length, APP.HEADERS.PEMBAYARAN.length).setValues(newRows);
      if (logRows.length) {
        const logSheet = getSheet_(APP.SHEETS.LOG);
        logSheet.getRange(logSheet.getLastRow() + 1, 1, logRows.length, APP.HEADERS.LOG.length).setValues(logRows);
      }
      SpreadsheetApp.flush();
      bumpPaymentCacheVersion_();
    }

    return { results: results, saved: newRows.length };
  } finally {
    lock.releaseLock();
  }
}
...
function bootstrapPaymentSnapshot_(periods, session) {
  const paidSets = {};
  periods.forEach(p => paidSets[periodKey_(p.bulan, p.tahun)] = new Set());
  const allowedClass = clean_(session && session.role).toUpperCase() === 'WALI' ? normalizeClassName_(session.kelas) : '';
  const payments = [];

  rowsAsObjects_(getSheet_(APP.SHEETS.PEMBAYARAN)).forEach(p => {
    if (clean_(p.STATUS).toUpperCase() !== 'LUNAS') return;
    const bulan = clean_(p.BULAN).toUpperCase();
    const tahun = Number(p.TAHUN);
    const key = periodKey_(bulan, tahun);
    const idSiswa = clean_(p.ID_SISWA);
    if (paidSets[key] && idSiswa) paidSets[key].add(idSiswa);

    const kelas = normalizeClassName_(p.KELAS);
    if (allowedClass && kelas !== allowedClass) return;
    payments.push({
      idTransaksi: clean_(p.ID_TRANSAKSI),
      idSiswa: idSiswa,
      bulan: bulan,
  ...
  return { paidSets: paidSets, payments: payments };
}
...
function ensurePenggunaSheet_(ss) {
  let sh = ss.getSheetByName(APP.SHEETS.PENGGUNA);
  if (!sh) sh = ss.insertSheet(APP.SHEETS.PENGGUNA);

  const lastCol = Math.max(1, sh.getLastColumn());
  const existingHeaders = sh.getRange(1, 1, 1, lastCol).getValues()[0].map(clean_);
  const hasExistingData = sh.getLastRow() > 1 || existingHeaders.some(Boolean);

  if (hasExistingData && !existingHeaders.includes('PASSWORD_SISWA')) {
    const passwordIndex = existingHeaders.indexOf('PASSWORD');
    if (passwordIndex >= 0) {
      sh.insertColumnAfter(passwordIndex + 1);
      sh.getRange(1, passwordIndex + 2).setValue('PASSWORD_SISWA');
    } else {
      throw new Error('Kolom PASSWORD pada sheet PENGGUNA tidak ditemukan.');
    }
  }

  sh.getRange(1, 1, 1, APP.HEADERS.PENGGUNA.length).setValues([APP.HEADERS.PENGGUNA]);
  sh.setFrozenRows(1);
  sh.getRange(1, 1, 1, APP.HEADERS.PENGGUNA.length)
